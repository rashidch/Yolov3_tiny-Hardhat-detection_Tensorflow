...
from utils.layer_utils import conv2d, darknet19_body, yolo_tiny_block, upsample_layer

class yolov3_tiny(object):

    def __init__(self, class_num, anchors, batch_norm_decay=0.9):

        # self.anchors = [[10, 18], [15, 25], [27, 36],
                         # [42, 68], [79, 134], [171, 272],
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm],reuse=reuse):
            with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                ...
                with tf.variable_scope('yolov3_tiny_head'):
                    ...
                    inter1 = upsample_layer(inter1, route_1.get_shape().as_list())
    ...
    def reorg_layer_xx(self, feature_map, anchors):
        '''
        feature_map: a feature_map from [feature_map_1, feature_map_2, feature_map_3] returned
            from `forward` function
        anchors: shape: [3, 2]
        ...
        feature_map_1, feature_map_2 = tf.split(feature_map, [3*4, 3*(self.class_num+1)], axis=-1)
        feature_map_1 = tf.reshape(feature_map_1, [-1, grid_size[0], grid_size[1], 3, 4])
        feature_map_2 = tf.reshape(feature_map_2, [-1, grid_size[0], grid_size[1], 3, 1 + self.class_num])
        ...
        # conf_logits: [N, 13, 13, 3, 1]
        # prob_logits: [N, 13, 13, 3, class_num]
        box_centers, box_sizes = tf.split(feature_map_1, [2, 2], axis=-1)
        conf_logits, prob_logits = tf.split(feature_map_2, [1, self.class_num], axis=-1)
        box_centers = tf.nn.sigmoid(box_centers)
    ...
    def predict(self, feature_maps):
        ...
        feature_map_anchors = [(feature_map_1, self.anchors[3:6]),
                               (feature_map_2, self.anchors[0:3])]
